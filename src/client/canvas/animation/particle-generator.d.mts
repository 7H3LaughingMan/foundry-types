 ...
 */
export type ParticleGeneratorAnchor = PlaceableObject | ParticleGeneratorPoint | (() => { x: number; y: number }) | null;
 ...
 */
export type ParticleGeneratorAnchorPoint = "center" | "position" | ((source: PlaceableObject) => { x: number; y: number });
...
export interface ParticleGeneratorConfiguration {
     ...
     * @defaultValue null
     */
    blur?: number | { intensity: number; quality?: number } | { enabled: boolean; intensity: number; quality?: number } | null;
    /**
...
type ParticleGeneratorPositionTest = (x: number, y: number, data: { generator: ParticleGenerator; particle: ParticleMesh }) => boolean;
